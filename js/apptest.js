...
barba.init({
  ...
  transitions: [
    /* TRANSITION FROM MAIN PAGE TO OTHER PAGES */
    {
      name: "swipemain",
      ...
      to: {
        namespace: ["transylvania", "trekking", "fashion"],
      },
      leave({ current, next }) {
        ...
        tl.fromTo(
          ...
          1,
          { x: "-100%" },
          { x: "0%", stagger: "0.30", onComplete: done },
          "-=0.5"
        );
      },
      enter({ current, next }) {
        ...
        /* animation */
        const tl = gsap.timeline({ defaults: { ease: "power2.inOut" } });
        tl.fromTo(".swipe", 0.25, { x: "0%" }, { x: "100%" });
        tl.fromTo(next.container, 1, { opacity: 0 }, { opacity: 1 }, "-=0.5");
      },

      /* TRANSITION FROM TRANSYLVANIA PAGE TO MAIN PAGE */
      name: "swipetransylvania",
      ...
      enter({ current, next }) {
        ...
        /* animation */
        const tl = gsap.timeline({ defaults: { ease: "power2.inOut" } });
        tl.fromTo(".swipe", 0.35, { x: "0%" }, { x: "-100%" });
        tl.fromTo(
          next.container,
          1,
          { opacity: 0 },
  ...
  //       /* animation */
  //       const tl = gsap.timeline({ defaults: { ease: "power2.inOut" } });
  //       tl.fromTo(".swipe", 1, { x: "0%" }, { x: "100%", stagger: "0.5" });
